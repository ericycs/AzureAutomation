...
<#
.SYNOPSIS
  Connects to Azure and starts of all VMs in the specified Azure subscription or resource group. Ignores VM in Exception Lists.
...
.NOTES
	...
	By: Eric Yew
	SOURCE: https://github.com/ericyew/AzureAutomation/blob/master/Start-AzureRmVM.ps1
#>
...
foreach ($VM in $VMsList) {
    ...
    {
        Write-Output ($VM.Name + " is already started.")
    }
    else
    {
        $StartSts = $VM | Start-AzureRmVM -ErrorAction Continue
